    ...
    <ScrollView style={{ top: top }}>
      <View className="flex-1 justify-center items-center bg-[#1E1E1E] mb-5">
        ...
        <TextInput
          ...
          placeholder={
            'Ingresa una breve descripción del libro que propones a cambio de él...'
          }
          multiline
        />
        ...
        <Text className="text-[#FFFFFF] text-base mb-2 mt-5">
          ¿Queres hacer un trueque o venderlo?
        </Text>
        <View className="flex-row mb-5">
          <TouchableOpacity
            onPress={() => setBarter(!barter)}
            style={{ elevation: 2 }}
            className={`px-4 py-4 m-2 rounded-xl text-xs flex-row items-center bg-[#${
              barter ? 'FF9A1F' : '1E1E1E'
            }]`}
            activeOpacity={0.8}
          >
            <Entypo
              name="shuffle"
              size={18}
              color={`#${barter ? '1E1E1E' : 'FF9A1F'}`}
            />
            <Text className={`text-[#${barter ? '1E1E1E' : 'FF9A1F'}] mx-2`}>
              Trueque
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          >
            <FontAwesome5
              name="coins"
              size={18}
              color={`#${sell ? '1E1E1E' : 'FF9A1F'}`}
            />
            <Text className={`text-[#${sell ? '1E1E1E' : 'FF9A1F'}] mx-2`}>
              Venta
            </Text>
          </TouchableOpacity>
        </View>
        ...
        >
          <Text className="text-[#1E1E1E] self-center">SIGUIENTE</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
