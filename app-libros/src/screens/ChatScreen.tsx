...
export const ChatScreen = ({ route }: Props) => {
  ...
  return (
    <View className="flex-1 bg-bookBlack">
      {/* Header */}
      <View
        className="border-b-2 border-black py-2 z-50 bg-bookBlack"
        style={{ top: top }}
      >
        <View className="container mx-auto px-4 flex-row items-center justify-between">
          <View className="flex-row items-center justify-center">
            <Image source={{ uri: image }} className="w-[42] h-[41]" />
            <Text className="font-medium text-base text-[#EBEBEB] mx-3">
              {user}
            </Text>
          </View>
